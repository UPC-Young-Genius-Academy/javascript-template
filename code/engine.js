...
function FlappyGame() {
    var INITIAL_GRAVITY = { enabled: false, amount: GAME_GRAVITY_RATE };
    var SPEED_LIMIT = GAME_SPEED_LIMIT;

    this.author = GAME_AUTHOR;
...
function FlappyEngine(game) {
    ...
    this.obstacleSpriteFromImage = function(imageId, regionRects, textureRects, velocity) {
        ...
        sprites.pipe.tileScale.x = 2 * region.width / tr.width;
        ...
        sprites.portal.tileScale.x = 2 * region.width / tr.width;
    ...
    this.parallaxSpriteFromImage = function(imageId, regionRect, scrollVelocity, textureRect) {
        var texture = this.createTexture(imageId);
